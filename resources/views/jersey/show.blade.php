...
<div class="mx-auto max-w-4xl px-4 py-14">
 ...
 @if($order->balance>0)<form method="post" enctype="multipart/form-data" action="{{route('jersey.payment',$order->order_number)}}" class="mt-10 rounded-xl bg-white p-6 shadow">@csrf<input type="hidden" name="phone" value="{{$order->phone}}"><h2 class="text-xl font-black">Upload Bukti Pembayaran</h2><div class="mt-4 grid gap-4 md:grid-cols-2"><input type="number" name="amount" min="1000" max="{{$order->balance}}" placeholder="Jumlah pembayaran" required class="rounded-lg border p-3"><input type="file" name="proof" accept="image/*" required class="rounded-lg border p-3"></div><button class="mt-4 rounded-lg bg-emerald-800 px-5 py-3 font-bold text-white">Kirim Bukti</button></form>@endif
</div>
@endsection
