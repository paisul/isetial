...
class FoundationTest extends TestCase
{
    ...
    public function test_2026_structure_seeder_populates_the_public_chart_idempotently(): void
    {
        ...
        $this->get(route('structure'))->assertOk()->assertSeeText('Solahudin Awae')->assertSeeText('Setiausaha')->assertSeeTextInOrder(['Mode HP', 'Mode Desktop', 'Setiausaha', 'Bidang-Bidang', 'Kegiatan'])->assertSee('setStructureView')->assertSeeText('Ihsan Yusoh');
    }
    ...
    public function test_customer_can_order_lookup_and_upload_private_jersey_payment(): void
    {
        ...
        $this->get(route('jersey.create'))->assertOk()->assertSee('Simpan Keranjang')->assertSee('Bayar Sekarang')->assertSee('id="checkout-sheet"', false)->assertSee('Data pemesan')->assertSee('Panduan ukuran')->assertSee('5XL')->assertDontSee('Cek status pesanan sebelumnya');
        $selection = ['jersey_product_id' => $product->id, 'model' => 'Lelaki Pendek', 'sleeve' => 'short', 'quantity' => 1, 'intent' => 'save_cart'];
        $this->postJson(route('jersey.cart.add'), [...$selection, 'jersey_size_id' => $product->sizes->first()->id])->assertOk()->assertJson(['cart_count' => 1])->assertJsonStructure(['line_id']);
        ...
        $this->get(route('jersey.cart'))->assertOk()->assertSee('Hapus')->assertSee('Checkout sekarang')->assertSee('fixed inset-x-0 bottom-0', false)->assertSee('id="checkout-sheet"', false)->assertSee('Data pemesan')->assertDontSee('Perbarui');
