...
<div class="mx-auto max-w-6xl px-4 py-10 {{ $items->isNotEmpty() ? 'pb-32 lg:pb-10' : '' }}">
    ...
    @else
        <div class="mt-8 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
            ...
            <aside class="sticky top-24 hidden overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-lg lg:block"><div class="p-5"><p class="text-xs font-black uppercase tracking-[.14em] text-emerald-700">Ringkasan belanja</p><div class="mt-5 space-y-3 text-sm"><div class="flex justify-between"><span>Total item</span><b>{{$items->sum(fn($e)=>$e['item']['quantity'])}} jersey</b></div><div class="flex justify-between border-t pt-4 text-lg"><span class="font-bold">Total</span><b class="text-emerald-900">฿{{number_format($items->sum(fn($e)=>$e['unit']*$e['item']['quantity']),0,'.',',')}}</b></div></div><a href="{{route('jersey.checkout')}}" class="mt-6 block rounded-xl bg-amber-400 px-5 py-3.5 text-center font-black text-emerald-950 hover:bg-amber-300">Checkout sekarang →</a><p class="mt-3 text-center text-xs text-slate-400">Pembayaran dilakukan setelah pesanan dibuat.</p></div></aside>
        </div>
        <div class="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 p-3 shadow-[0_-8px_24px_rgba(15,23,42,0.12)] backdrop-blur lg:hidden">
            <div class="mx-auto flex max-w-6xl items-center gap-3">
                ...
                <a href="{{route('jersey.checkout')}}" class="shrink-0 rounded-xl bg-amber-400 px-5 py-3.5 text-center font-black text-emerald-950 hover:bg-amber-300">Checkout sekarang →</a>
            </div>
        </div>
    @endif
</div>
