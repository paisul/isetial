...
<div class="mx-auto max-w-4xl px-4 py-14">
 ...
 <div class="mt-7 grid gap-4 md:grid-cols-3"><div class="rounded-xl bg-white p-5 shadow"><small>Total</small><p class="text-2xl font-bold">฿{{number_format($order->total,0,'.',',')}}</p></div><div class="rounded-xl bg-white p-5 shadow"><small>Sudah dibayar</small><p class="text-2xl font-bold">฿{{number_format($order->paid_amount,0,'.',',')}}</p></div><div class="rounded-xl bg-emerald-900 p-5 text-white"><small>Sisa</small><p class="text-2xl font-bold">฿{{number_format($order->balance,0,'.',',')}}</p><b>{{$order->payment_status}}</b></div></div>
 <h2 class="mt-10 text-2xl font-black">Riwayat Pembayaran</h2>
 @forelse($order->payments as $p)<div class="mt-3 rounded-lg bg-white p-4">฿{{number_format($p->amount,0,'.',',')}} · <b>{{$p->status}}</b> · {{$p->created_at->format('d/m/Y')}}</div>@empty<p class="mt-3">Belum ada pembayaran.</p>@endforelse
